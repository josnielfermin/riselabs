...
export const Products = ({ onHover, onLeave }: ProductsProps) => {
  return (
    <section className="flex flex-col justify-between h-[441px] relative -top-10">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-lg font-normal text-base-2">
      ...
      <div className="flex flex-col">
        {Content.products.items.map((item, index) => (
          ...
          >
            ...
            <span
              className={`icon-arrow-right text-2xl group-hover:opacity-100 opacity-0 *:transition-all !duration-300`}
            ></span>
            <Image
              src={"/static/images/product-gradient.png"}
              alt={""}
              fill
              className="w-full h-full absolute z-[-1] group-hover:opacity-100 opacity-0 *:transition-all !duration-300"
            />
          </div>
        ))}
      </div>
